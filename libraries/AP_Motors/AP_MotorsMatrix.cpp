...
// output_armed - sends commands to the motors
// includes new scaling stability patch
void AP_MotorsMatrix::output_armed_stabilizingadp()
{
    uint8_t i;                          // general purpose counter
    float   roll_thrust;                // roll thrust input value, +/- 1.0
    float   pitch_thrust;               // pitch thrust input value, +/- 1.0
    ...
    pitch_thrust = _pitch_in * get_compensation_gain();
    yaw_thrust = _yaw_in * get_compensation_gain();
    //throttle_thrust = _throttleadp * get_compensation_gain();
    throttle_thrust = get_throttle() * get_compensation_gain();
    ...
    _omega1sq_adp=_thrust_rpyt_out[0];
    _omega2sq_adp=_thrust_rpyt_out[1];
    _omega3sq_adp=_thrust_rpyt_out[2];
    _omega4sq_adp=_thrust_rpyt_out[3];
}
